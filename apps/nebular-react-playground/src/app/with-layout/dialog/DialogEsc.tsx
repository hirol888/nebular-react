...
const DialogEsc: React.FC = () => {
  const dialogRef = useRef<NbDialogRef>(null);

  const config = new NbDialogConfig();

  const openWithEscClose = () => {
    dialogRef.current?.open({ ...config, closeOnEsc: true });
  };

  const openWithoutEscClose = () => {
    config.closeOnEsc = false;
    dialogRef.current?.open({ ...config, closeOnEsc: false });
  };
  ...
  return (
    <>
      <NbButton onClick={openWithEscClose}>Open with escape close</NbButton>
      <NbButton onClick={openWithoutEscClose}>Open without escape close</NbButton>
