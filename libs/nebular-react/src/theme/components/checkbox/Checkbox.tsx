...
import { NbComponentOrCustomStatus } from '@nebular-react';
 ...
 *
 * @stacked-example(Showcase, checkbox/checkbox-showcase)
 *
 * ### Usage
 *
 * Checkbox is available in multiple colors using `status` property:
 * @stacked-example(Colored Checkboxes, checkbox/checkbox-status)
 *
 * Indeterminate state is also supported:
 * @stacked-example(Indeterminate Checkbox, checkbox/checkbox-indeterminate)
 *
 * Checkbox can be disabled via `disabled` attribute.
 * @stacked-example(Disabled Checkbox, checkbox/checkbox-disabled)
 *
 * @styles
 *
 * checkbox-height:
 ...
 */
const NbCheckbox = React.forwardRef<CheckboxRef, CheckboxProps & React.HTMLAttributes<HTMLDivElement>>(
  (
    {
      checked = false,
      disabled = false,
    ...
    const [indeterminateValue, setIndeterminateValue] = useState<boolean>(indeterminate);
    // #endregion
    ...
    return (
      <div className={classNames('nb-checkbox', 'nb-transition', `status-${status}`, className)} {...otherProps}>
        <label className="label">
          ...
          >
            {indeterminateValue && (
              <NbIcon icon="minus-bold-outline" pack="nebular-essentials" className="custom-checkbox-icon"></NbIcon>
            )}
            {checkedValue && !indeterminateValue && (
              <NbIcon icon="checkmark-bold-outline" pack="nebular-essentials" className="custom-checkbox-icon"></NbIcon>
            )}
          </span>
