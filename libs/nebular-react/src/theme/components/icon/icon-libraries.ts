...
import { NbEvaSvgIcon, NbFontIcon, NbIconModel, NbOriginalEvaIcon, NbSvgIcon } from './_icon';
...
export class NbIconLibraries {
  ...
  protected defaultPack: NbIconPack | undefined;

  constructor() {
    this.registerSvgPack('eva', this.createIcons());
    this.registerSvgPack('nebular-essentials', {
      'chevron-down-outline':
        '<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" width="100%" height="100%" viewBox="0 0 24 24"><g data-name="Layer 2"><g data-name="chevron-down"><rect width="24" height="24" opacity="0"/><path d="M12 15.5a1 1 0 0 1-.71-.29l-4-4a1 1 0 1 1 1.42-1.42L12 13.1l3.3-3.18a1 1 0 1 1 1.38 1.44l-4 3.86a1 1 0 0 1-.68.28z"/></g></g></svg>',
      'chevron-up-outline':
      ...
      'minus-outline':
        '<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" width="100%" height="100%" viewBox="0 0 24 24"><g data-name="Layer 2"><g data-name="minus"><rect width="24" height="24" transform="rotate(180 12 12)" opacity="0"/><path d="M19 13H5a1 1 0 0 1 0-2h14a1 1 0 0 1 0 2z"/></g></g></svg>',
      'minus-bold-outline':
        '<svg xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" viewBox="0 0 8 2" width="100%" height="100%"><defs><rect id="nb-mbo" y="3" width="8" height="2" rx="1"/></defs><use xlink:href="#nb-mbo" transform="translate(0 -3)" fill-rule="evenodd"/></svg>',
      'checkmark-bold-outline':
        '<svg xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" viewBox="0 0 8 7" width="100%" height="100%"><defs><path id="nb-cbo" d="M6.039 1.43a1.11 1.11 0 0 1 1.517-.228c.483.342.588.998.234 1.466L4.431 7.1a1 1 0 0 1-1.492.115L.317 4.677a1.023 1.023 0 0 1 .002-1.483 1.113 1.113 0 0 1 1.535.002l1.641 1.59L6.04 1.428z"/></defs><use xlink:href="#nb-cbo" transform="translate(0 -1)" fill-rule="evenodd"/></svg>',
      'arrow-back':
        '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><g data-name="Layer 2"><g data-name="arrow-back"><rect width="24" height="24" transform="rotate(90 12 12)" opacity="0"/><path d="M19 11H7.14l3.63-4.36a1 1 0 1 0-1.54-1.28l-5 6a1.19 1.19 0 0 0-.09.15c0 .05 0 .08-.07.13A1 1 0 0 0 4 12a1 1 0 0 0 .07.36c0 .05 0 .08.07.13a1.19 1.19 0 0 0 .09.15l5 6A1 1 0 0 0 10 19a1 1 0 0 0 .64-.23 1 1 0 0 0 .13-1.41L7.14 13H19a1 1 0 0 0 0-2z"/></g></g></svg>'
    });
    this.defaultPack = this.packs.get('eva');
  }

  /**
   * Registers new Svg icon pack
   * @param {string} name
   * @param {NbIconModel} icons
   ...
   * @param {string} name
   */
  getPack(name: string): NbIconPack | undefined {
  ...
  protected createSvgIcon(name: string, content: NbIconModel | string, params: NbIconPackParams): NbSvgIcon {
    return content instanceof NbSvgIcon ? content : new NbSvgIcon(name, content, params);
  }

  protected createFontIcon(name: string, content: NbIconModel | string, params: NbFontIconPackParams): NbFontIcon {
  ...
  protected getPackOrThrow(name: string): NbIconPack {
    ...
    const pack: NbIconPack | undefined = this.packs.get(name);
    if (!pack) {
      throwPackNotFoundError(name);
    }
    return pack!;
  ...
  protected getIconFromPack(name: string, pack: NbIconPack): NbIconModel | string | null {
  ...
  private createIcons(): NbIcons {
    return Object
      .entries<NbOriginalEvaIcon>(eva.icons)
      .map(([name, icon]) => {
        return [name, new NbEvaSvgIcon(name, icon)] as [string, NbSvgIcon];
      })
      .reduce((newIcons: any, [name, icon]: [string, NbSvgIcon]) => {
        newIcons[name] = icon;
        return newIcons;
      }, {});
  }
