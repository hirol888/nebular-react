...
import { NbPositionStrategy } from './position-strategy';
import { OverlayReference } from '../overlay-reference';
import { NbConnectedOverlayPositionChange, NbConnectionPositionPair, ScrollingVisibility } from './connected-position';
...
import { ViewportRuler, ViewportScrollPosition } from '../../scrolling';
import { containerClass, Dimensions, extendStyles, getPixelValue, getRoundedBoundingClientRect, NbLayoutDirection } from '../../helper';
 ...
 * of the overlay.
 */
export class NbFlexibleConnectedPositionStrategy implements NbPositionStrategy {
  ...
  _preferredPositions: NbConnectionPositionPair[] = [];

  /** The origin element against which the overlay will be positioned. */
  private _origin: HTMLElement | (Point & { width?: number; height?: number; });
  ...
  private _lastPosition: NbConnectedPosition | null;

  /** Subject that emits whenever the position changes. */
  private readonly _positionChanges = new Subject<NbConnectedOverlayPositionChange>();
  ...
  positionChanges: Observable<NbConnectedOverlayPositionChange> = this._positionChanges;

  /** Ordered list of preferred positions, from most to least desirable. */
  get positions(): NbConnectionPositionPair[] {
  ...
  apply(): void {
    ...
    this._containerRect = document.querySelector(`.${containerClass}`)!.getBoundingClientRect();
   ...
   */
  withPositions(positions: NbConnectedPosition[]): this {
   ...
   */
  setOrigin(origin: HTMLElement | (Point & { width?: number; height?: number; })): this {
    this._origin = origin;
    return this;
  }
   ...
   */
  private _applyPosition(position: NbConnectedPosition, originPoint: Point) {
    ...
    if (this._positionChanges.observers.length) {
      ...
      const changeEvent = new NbConnectedOverlayPositionChange(position, scrollableViewProperties);
  ...
  private _setTransformOrigin(position: NbConnectedPosition) {
   ...
   */
  private _calculateBoundingBoxRect(origin: Point, position: NbConnectedPosition): BoundingBoxRect {
    const viewport = this._viewportRect;
   ...
   * @param position The position preference
   */
  private _setBoundingBoxStyles(origin: Point, position: NbConnectedPosition): void {
  ...
  private _setOverlayElementStyles(originPoint: Point, position: NbConnectedPosition): void {
    const styles = {} as CSSStyleDeclaration;
  ...
  private _isRtl() {
    return this._overlayRef.getDirection() === NbLayoutDirection.RTL;
  }
  ...
  private _getOffset(position: NbConnectedPosition, axis: 'x' | 'y') {
    if (axis === 'x') {
...
export interface NbConnectedPosition {
